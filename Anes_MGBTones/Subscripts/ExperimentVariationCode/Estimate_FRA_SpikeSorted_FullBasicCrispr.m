...
% Estimates FRA for whisk and control condition from Crispr Jaws animals
% in exps 62-70

clearvars -except Exp ExpCount Thresh ExpNo BasicExtract CurPath attenuation smoothing
%close all
if ExpCount==11
    basedir=['DATA' filesep 'MGB_Tones' filesep sprintf('expt%d',Exp(ExpCount).ExpNo) filesep sprintf('P0%d-tone_with_WhiskandOpto.2',Exp(ExpCount).Pen)];
elseif ExpCount==14
    basedir=['DATA' filesep 'MGB_Tones' filesep sprintf('expt%d',Exp(ExpCount).ExpNo) filesep sprintf('P0%d-tone_with_WhiskandOpto.2',Exp(ExpCount).Pen)];
    
else
    basedir=['DATA' filesep 'MGB_Tones' filesep sprintf('expt%d',Exp(ExpCount).ExpNo) filesep sprintf('P0%d-tone_with_WhiskandOpto',Exp(ExpCount).Pen)];
end
...
noise_times=unique(grid.stimGrid(:,3))/10; % divided by ten so it matches the PSTH bins
...
noise_dur=5; % the window is ten times bigger than this number (edges are 10ms)
response_win_delay=1;% the dedit elay is ten times bigger than this number (edges are 10ms)

Exp(ExpCount).noise_dur=noise_dur;
...
for electrode =1:length(elec)
    ...
    for cur_set_act=1:sets
        ...
        mean_PSTH_sets(cur_set,:) = mean_PSTH;
        
        spont_resp_win=[2:10];
        spont_response_win_freqs_10ms_spacing=(mean_PSTH(:,spont_resp_win));
        spont_response_win_freqs(cur_set)=mean(spont_response_win_freqs_10ms_spacing);
        
        
    end
    % Average spontanous rate across all trials
    AvgSpont=mean(spont_response_win_freqs);
    ...
    for cur_set_act=1:sets
        ...
        set_resp_win=[(grid.stimGrid(cur_set_act,3)/10+response_win_delay):((grid.stimGrid(cur_set_act,3)/10)+noise_dur+response_win_delay)];
        ...
        response_win_freqs(cur_set)=mean(response_win_freqs_10ms_spacing);%-AvgSpont; % find peak in reponse window
    ...
    for condition=1
        ...
        n=0;
        for FreqCount=1:length(freqs); % minus one bcause there is a spontanous activity sweep
            for levelCount=1:length(levels); % minus one bcause there is a spontanous activity sweep
                n=n+1;
    ...
    for condition=6 %control
        ...
        temp=(length(freqs))*(length(levels));
        if length(condIdx)==length(temp);
            disp('dimesions match')
        end
        n=0;
    ...
    BF80All(electrode)=freqs(BF80AllId(electrode));
    BF60All(electrode)=freqs(BF60AllId(electrode));
    
    subplot(8,8,iiii)
    AvgSpontAll(electrode)=AvgSpont;
...
Exp(ExpCount).Spont=AvgSpontAll



%
% figure
% estimatefrequencytuningchanges_olsen_stylev3